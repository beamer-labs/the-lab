	...
	this.takeAction = function(map)
	{
		...
		if(this.health > 0)
		{
			...
			for(var x =0;x < miniMap.length;x++)
			{
				...
				{
					inputs.push(miniMap[x][y]/100);
				}
			}

			inputs.push(this.age/100);
			inputs.push(this.health/100);
			inputs.push(this.map.has(map._FOOD));

			this.lastInputs = inputs;
			this.currActionStr = "";

			if(this.currActionIdx >= this.currOutput.length)
			{
